...
The command entrypoints point to these custom workflows:

- `.cursor/commands/bmad-bmm-sprint-planning.md` / `.claude/commands/bmad-bmm-sprint-planning.md` — sprint planning and Linear sync
- `.cursor/commands/bmad-bmm-dev-from-linear.md` — implement Linear issues using BMAD stories (see below)
...
## Implementing from Linear issues

Use the `bmad-bmm-dev-from-linear` command to implement one or more Linear issues using their corresponding BMAD stories. The command resolves Linear IDs (e.g. `TASK-253`) to BMAD keys via `_bmad-output/implementation-artifacts/linear-issue-map.yaml`, creates story files, and implements them.

**Usage** (in Cursor or Claude chat):

```plaintext
/bmad-bmm-dev-from-linear TASK-253
```

Or multiple issues:

```plaintext
/bmad-bmm-dev-from-linear TASK-253 TASK-254
```

Requires `linear-issue-map.yaml` (generated by sprint-planning when `tracking_system: linear`). BMAD is the source of truth; Linear is the execution/visibility layer.
